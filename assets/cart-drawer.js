class CartDrawer {
  ...
  async renderCart() {
          ...
          <div id="shopify-installments" data-testid="shopify-installments" class="m-0 font-system flex flex-col gap-2" style="margin-bottom: 16px;">
            <p style="margin: 0; font-size: 14px; line-height: 1.5;">
              <span id="shopify-installments-content" data-testid="shopify-installments-content" class="pr-1.5" style="padding-right: 6px;">
                4 interest-free installments, or from <span class="font-bold" style="font-weight: 600;">${this.formatMoney(
                  this.cartData.total_price / 4
                )}</span>/mo with 
    ...
    this.bindCartEvents();
    this.initializeShopPayInstallments();
  }

  initializeShopPayInstallments() {
    // Initialize Shop Pay Installments if the script is available
    // The Shop Pay script should automatically detect and initialize the element
    // But we ensure it's ready after a short delay
    setTimeout(() => {
      const installmentsElement = this.drawer.querySelector("#shopify-installments");
      if (installmentsElement && window.Shopify && window.Shopify.PaymentButton) {
        try {
          if (typeof window.Shopify.PaymentButton.init === "function") {
            window.Shopify.PaymentButton.init();
          }
        } catch (e) {
          console.log("Shop Pay Installments initialization:", e);
        }
      }
    }, 100);
  }
